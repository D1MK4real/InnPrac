...
def process_cover_to_palette(f: str, checkpoint_root: str, cover_dir: str, palette_count: int,
    ...
    if not os.path.isfile(palette_tensor_f):
        ...
        palette_tensor = torch.load(palette_tensor_f)
...
class PaletteDataset(Dataset[Tuple[torch.Tensor, torch.Tensor]]):
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        art_style = self.data.iloc[index]['art_style']
        name = self.data.iloc[index]['painting']
        # cover_path = os.path.join(self.cover_dir_, art_style, name+'.jpg')

        palette_tensor = process_cover_to_palette(name, self.checkpoint_root_, os.path.join(self.cover_dir_, art_style),
